...
public class Kategori1841720002Andy {
    ...
    ArrayList<Kategori1841720002Andy> getByNamaAndKeteranganAndy(String nama, String keterangan) {
        ArrayList<Kategori1841720002Andy> ListKategori = new ArrayList();
        ResultSet rs;
        if ((nama.trim().length()>0) && (keterangan.trim().length()>0)){
            rs = DBHelper1841720002Andy.selectQueryAndy("SELECT * FROM kategori Where nama = '"+
            nama +"' and keterangan = '"+ keterangan+"'");
        }else if ((nama.trim().length()>0) && (keterangan.trim().length()==0)){
            rs = DBHelper1841720002Andy.selectQueryAndy("SELECT * FROM kategori Where nama = '"+
            nama+"'");
        }else {
            rs = DBHelper1841720002Andy.selectQueryAndy("SELECT * FROM kategori Where keterangan = '"+ keterangan+"'");
        }
        try {
            while (rs.next()) {
                Kategori1841720002Andy kat = new Kategori1841720002Andy();
                kat.setIdKategoriAndy(rs.getInt("idkategori"));
                kat.setNamaAndy(rs.getString("nama"));
                kat.setKeteranganAndy(rs.getString("keterangan"));
